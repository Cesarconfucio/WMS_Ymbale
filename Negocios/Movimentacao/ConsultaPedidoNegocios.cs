...
namespace Negocios
{
    public class ConsultaPedidoNegocios
    {
        ...
        //Pesquisa o pedido
        public Pedido PesqPedido(int codPedido)
        {
            try
            {
                //Instância o objêto
                ...
                string select = "select ped_data, ped_codigo, cf.cfop_codigo, cf.cfop_descricao, tp.tipo_codigo, tp.tipo_descricao, " +
                                ...
                                "on cf.cfop_codigo = ped.cfop_codigo " +
                                "left join wms_tipo_pedido tp " +
                                "on tp.tipo_codigo = ped.tipo_codigo " +
                                "left join wms_prazo pr " +
                                "on pr.prazo_codigo = ped.prazo_codigo " +
                                "left join wms_pagamento pg " +
                                "on pg.pag_codigo = ped.pag_codigo " +
                                "left join wms_representante rep " +
                                "on rep.rep_codigo = ped.rep_codigo " +
        ...
        //Pesquisa o pedido
        public ItensPedidoCollection PesqItem(int codPedido)
        {
            try
            {
                //Instância o objêto
                ...
                "where sep_tipo = 'CAIXA' and prod_id = pp.prod_id order by apa_ordem) as ordem_separacao, " +
                "(select a.apa_endereco from wms_separacao s " +
                "inner join wms_apartamento a " +
                "on a.apa_codigo = s.apa_codigo " +
                "where sep_tipo = 'CAIXA' and prod_id = pp.prod_id) as apa_endereco_cxa, " +
                "(select a.apa_endereco from wms_separacao s " +
                "inner join wms_apartamento a " +
                "on a.apa_codigo = s.apa_codigo " +
                "where sep_tipo = 'FLOWRACK' and prod_id = pp.prod_id) as apa_endereco_flow, " +
