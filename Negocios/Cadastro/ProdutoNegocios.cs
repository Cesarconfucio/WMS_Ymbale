    ...
    public class ProdutoNegocios : EstruturaNegocios
    {
        ...
        public ProdutoCollection PesqProduto(string codProduto, string descProduto, string codFornecedor, string codBarra, string descCategoria, bool status, string empresa)
        {
            try
            {
                ...
                "p.prod_palete_blocado, p.prod_palete_padrao, p.prod_peso_variavel " +
                "from wms_produto p " +
                ...
                else if (codBarra != string.Empty)
                {
                    select += " inner join wms_barra b " +
                        ...
                        "where prod_status = @status and bar_numero = @codBarra and p.conf_codigo = (select conf_codigo from wms_configuracao where conf_sigla = @empresa) order by p.prod_codigo";
                }
        ...
        public void Alterar(Produto produto)
        {
            try
            {
                ...
                string update = "update wms_produto set cat_codigo = @codCategoria, prod_tipo_armazenamento = @tipoArmazenagem, prod_tipo_palete = @tipoPalete, prod_vida_util = @shelfLife, prod_tolerancia = @tolerancia, " +
                        ...
                        "prod_palete_blocado = @paleteBlocado, prod_palete_padrao = @paletePadrao, prod_peso_variavel = @pesoVariavel " +
        ...
        public BarraCollection PesqCodBarra(bool status)
        {
            try
            {
                //Instância a coleção
        ...
        public void SalvarCodBarra(int idProduto, string numeroBarra, int multiplicador,  double altura, double largura, double comprimento, double cubagem, double peso)
        {
            try
            {
                //Limpar Paramêtros
                ...
                //String de atualização
                string update = "insert into wms_barra (bar_codigo, prod_id, bar_numero, bar_multiplicador, bar_altura, bar_largura, bar_comprimento, bar_cubagem, bar_peso, bar_tipo) " +
                                "Values "+
                                "(gen_id(gen_wms_barra,1), @idProduto, @numeroBarra, @multiplicador, @altura, @largura, @comprimento, @cubagem, @peso, 'INTERNO')";
        ...
        public void AlterarCodBarra(int idProduto, int codBarra, double altura, double largura, double comprimento, double cubagem, double peso)
        {
            try
            {
                //Limpar Paramêtros
                ...
                string update = "update wms_barra set bar_altura = @altura, bar_largura = @largura, bar_comprimento = @comprimento, " +
                                ...
                                "where prod_id = @idProduto and bar_codigo = @codBarra";
