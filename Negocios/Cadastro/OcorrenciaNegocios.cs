    ...
    public class OcorrenciaNegocios
    {
        ...
        public void SalvarOcorrencia(Ocorrencia ocorrencia, string empresa)
        {
            try
            {
                ...
                string insert1 = "insert into wms_item_ocorrencia (poco_codigo, ioco_codigo, ped_codigo, prod_id, prod_quantidade, conf_codigo) " +
                                "select @codOcorrencia, gen_id(gen_wms_item_ocorrencia, 1), ped_codigo, prod_id, iped_quantidade," +
                                "(select conf_codigo from wms_configuracao where conf_sigla = @empresa) " +
        ...
        //Relatório
        public OcorrenciaCollection ImpressaoOcorrencia(string devolucao, string reentrega, string dataInicial, string dataFinal)
        {
            try
            {
                //Instância o objêto
                ...
                conexao.LimparParametros();
                //Adiciona os parâmetros
                conexao.AdicionarParamentros("@devolucao", devolucao);
                conexao.AdicionarParamentros("@reentrega", reentrega);
                conexao.AdicionarParamentros("@dataInicial", dataInicial + " 00:00:01");
                conexao.AdicionarParamentros("@dataFinal", dataFinal + " 23:59:59");
                ...
                if (!devolucao.Equals(string.Empty))
                {
                    select += "where poco_devolucao = @devolucao and poco_data_ocorrencia between @dataInicial and @dataFinal";
                }

                if (!reentrega.Equals(string.Empty))
                {
                    select += "where poco_reentrega = @reentrega  and poco_programacao_reentrega between @dataInicial and @dataFinal";
                }
