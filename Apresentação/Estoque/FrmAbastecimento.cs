        ...
        private void AnalisaAbastecimentoCaixa()
        {
            try
            {
                ...
                Invoke((MethodInvoker)delegate ()
                {
                    ...
                    itensCollection = abastecimentoNegocios.PesqItens(cmbTipo.Text.ToUpper(), cmbRegiao.Text, cmbRua.Text, cmbLado.Text, cmbCategoria.Text, txtCodFornecedor.Text, null); //A coleção recebe o resultado da consulta 
                });
        ...
        private void AnalisarAbastecimentoFlowRack()
        {
            try
            {
                ...
                Invoke((MethodInvoker)delegate ()
                {
                    ...
                    if (gridEstacao.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; gridEstacao.Rows.Count > i; i++)
                        {
                            ...
                            {
                                codigoEstacao[i] = Convert.ToInt32(gridEstacao.Rows[i].Cells[1].Value); //Passa o código da rota
                ...
                else
                {
                    ...
                    {
                        //Pesquisa o id dos Produtos que precisam de abastecimento
                        itensCollection = abastecimentoNegocios.PesqItens(cmbTipo.Text.ToUpper(), cmbRegiao.Text, cmbRua.Text, cmbLado.Text, cmbCategoria.Text, txtCodFornecedor.Text, codigoEstacao); //A coleção recebe o resultado da consulta 
                    });
                            ...
                            else
                            {
                                ...
                                for (int ii = 0; itensCollection[i].qtdAbastecer > qtdEncontrada; ii++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            //Altera a menssagem 
                                            itensCollection[i].observacao = "NÃO POSSUI PICKING DE GRANDEZA";
                                            qtdEncontrada = 0;
                                            //Adiciona o produto a coleção com problemas ou observações
                                            itensObservacaoCollection.Add(itensCollection[i]);
                                            //Pula para o próximo produto
                                ...
                                {
                                    //MessageBox.Show("Linha " + iii + " ID " + itensCollection[i].idProduto);
